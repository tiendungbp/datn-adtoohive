...
export default function BillList() {
	...
	const handleSearchByDate = (date) => {
		const list = billList.filter(
			(bill) => moment(bill.createdAt).format('YYYY-MM-DD') === date,
		);
		setSearchList(list);
	};
	...
	const handleSearchByBillID = () => {
		if (billKeyword) {
			if (
				billKeyword.length === 10 &&
				billKeyword.toLowerCase().slice(0, 2) === 'hd'
			) {
	...
	const handleSearchByPatientInfo = () => {
		if (patientKeyword) {
			...
			if (isPhoneNumber) {
				...
				patientKeyword.length === 10 &&
				patientKeyword.toLowerCase().slice(0, 2) === 'bn'
			) {
