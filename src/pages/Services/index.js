...
import { Button, Select, Modal, Form, Input, Radio, Spin, InputNumber } from "antd";
...
export default function Services() {
    ...
    const columns = [
        ...
        },
        {
            title: "Danh mục",
            dataIndex: "Category",
            render: category => CommonUtils.capitalizeEachWord(category.category_name)
        },
        {
            title: "Tên dịch vụ",
    ...
    const getActiveCategories = () => {
        setActiveCategoryList(categoryList.filter(category => category.status === 1));
    };
    ...
    const handleFilterByCategory = (category_id) => {
        if(category_id === -1) {
            ...
            const list = serviceList.filter(service => service.Category.category_id === category_id);
            setSearchList(list);
        };
    };
    ...
    const handleSearchByInfo = () => {
        if(keyword) {
            ...
            if(keyword.length === 10 && keyword.toLowerCase().slice(0, 2) === "dv") {
                ...
                list = serviceList.filter(service => {
                    return service.service_name.toLowerCase().includes(keyword.toLowerCase());
                });
            };
            setSearchList(list);
            setKeyword("");
