...
export default function Services({ accessToken }) {
	...
	const getActiveCategories = () => {
		setActiveCategoryList(
			categoryList.filter((category) => category.status === true),
		);
	};
	...
	//XỬ LÝ TÌM DỊCH VỤ THEO TÊN
	const handleSearchByName = () => {
		if (keyword) {
			const list = serviceList.filter((service) => {
