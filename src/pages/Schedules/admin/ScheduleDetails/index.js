...
export default function ScheduleDetails() {
		...
		},
		{
			...
			render: (obj) => (
				<Popconfirm
					...
					className={`${
						obj.UserSchedule.status || obj.date < currentDate
							? 'border-0 btn-disabled'
							: ''
					}`}
					title="Duyệt lịch?"
					cancelText="Hủy"
		...
		},
		{
			...
			render: (obj) => (
				<Popconfirm
					...
					className={`${
						obj.UserSchedule.status === 2 ? 'border-0 btn-disabled' : ''
					}`}
					title="Xóa lịch?"
					cancelText="Hủy"
					okText="Xóa"
	...
	const handleAcceptOne = async (user_schedule_id) => {
		setIsLoading(true);
		const res = await scheduleAPI.acceptOne({
	...
	const handleAcceptAll = () => {
		Swal.fire({
			...
			if (result.isConfirmed) {
				let list = scheduleList.filter(
					(schedule) => schedule.UserSchedule.status === 0,
				);
				list = list.map((schedule) => schedule.UserSchedule.user_schedule_id);

				setIsLoading(true);
				const res = await scheduleAPI.acceptAll({
					user_id: user.user_id,
					list,
				});
				setIsLoading(false);
