...
export default function CreateSchedule() {
			...
			render: (user_id, obj) =>
				obj.user_schedule_id ? (
					<Tooltip title="Xem chi tiết" placement="bottom">
						<Link to={`/lich-lam-viec/${user_id}/${selectedDate}`}>
							{user_id.toUpperCase()}
						</Link>
					</Tooltip>
				) : (
	...
	const getAllBySchedule = async () => {
		setIsLoading(true);
		const resDoctor = await doctorAPI.getAllBySchedule(
			selectedDate,
			selectedSession,
		);
		const resEmployee = await employeeAPI.getAllBySchedule(
			selectedDate,
			selectedSession,
		);

		console.log(resDoctor.data.data);
									...
									<div className="col-md mt-3">
										<div className="table-responsive">
											<Table
												...
												// rowClassName={(record, index) => (record.user_schedule_id > 50 ? "antd-bg-light" : "")}
											/>
										</div>
									</div>
								</div>
