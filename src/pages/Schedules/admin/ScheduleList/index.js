					...
					obj.schedules.map((schedule, index) => {
						if (schedule.date === item.value) {
							if (schedule.list.length) {
								const notAccepted = schedule.list.find(
									(listItem) => !listItem.UserSchedule.status,
								);
								if (notAccepted) {
									return (
										<Link
											key={index}
											to={`${detailsPage}/${obj.user.user_id}/${item.value}`}
										>
	...
	const handleFilterByRole = (role) => {
		...
		switch (role) {
			case 0:
				...
				break;
			case 2:
				list = scheduleList.filter(
					(item) => item.user.user_id.slice(0, 2) === 'qt',
				);
				setSearchList(list);
				break;
			case 3:
				list = scheduleList.filter(
					(item) => item.user.user_id.slice(0, 2) === 'lt',
				);
				setSearchList(list);
				break;
			case 4:
				list = scheduleList.filter(
					(item) => item.user.user_id.slice(0, 2) === 'bs',
				);
				setSearchList(list);
				break;
			case 5:
				list = scheduleList.filter(
					(item) => item.user.user_id.slice(0, 2) === 'pt',
				);
				setSearchList(list);
				break;
			default:
						...
						<div className="rounded p-4 bg-secondary">
							<Form layout="vertical">
								<div className="row mb-2">
									...
									<div className="col-md-4">
										<Form.Item label=" ">
											<Button onClick={handleAcceptForWeek}>
												Duyệt lịch của tuần
