...
export default function Report() {
    ...
    const serviceData = {
        labels: serviceList.map(s => CommonUtils.capitalizeEachWord(s.Service.service_name)),
        datasets: [{
            label: "Lượt dùng",
    ...
    const appointmentData = {
        labels: appointmentList.map(appointment => moment(appointment.date).format("DD/MM")),
        datasets: [
            {
                label: "Đặt mới",
                data: appointmentList.map(appointmentByDate => appointmentByDate.new.length),
                backgroundColor: "rgba(53, 162, 235, 0.5)"
            },
            {
                label: "Tái khám",
                data: appointmentList.map(appointmentByDate => appointmentByDate.reExam.length),
    ...
    const handleGetReportScheduleByMonth = async() => {
        if(monthSchedule) {
            ...
            fileDownload(res.data, `LichLamViec_${moment(monthSchedule).format("MM-YYYY")}.xlsx`);
            setMonthSchedule("");
            setIsLoading(false);
        }
        else {
    ...
    const handleGetReportServiceByMonth = async() => {
        if(monthService) {
            ...
            fileDownload(res.data, `DichVu_${moment(monthService).format("MM-YYYY")}.xlsx`);
            setMonthService("");
            setIsLoading(false);
        }
        else {
    ...
    const handleGetReportAppointmentByMonth = async() => {
        if(monthAppointment) {
            ...
            fileDownload(res.data, `LichHen_${moment(monthAppointment).format("MM-YYYY")}.xlsx`);
            setMonthAppointment("");
            setIsLoading(false);
        }
        else {
    ...
    const handleGetReportRevenueByMonth = async() => {
        if(monthRevenue) {
            ...
            fileDownload(res.data, `DoanhThu_${moment(monthRevenue).format("MM-YYYY")}.xlsx`);
            setMonthRevenue("");
            setIsLoading(false);
        }
        else {
                        ...
                        <div className="col-md">
                            <div className="row mb-4">
                                <div className="col-md-4">
                                    <Card
                                        title={`DOANH THU - ${currentRevenue.date ? currentRevenue.date : ""}`}
                                        style={{backgroundColor: "rgba(53, 162, 235, 0.5)"}}
                                    >
                                        <h3 className="text-white">{currentRevenue.total}</h3>
                                    </Card>
                                </div>
                                <div className="col-md-4">
                                    <Card
                                        title={`LỊCH HẸN MỚI - ${currentAppointment.date ? currentAppointment.date : ""}`}
                                ...
                                <div className="col-md-4">
                                    <Card
                                        title={`BỆNH NHÂN MỚI - ${currentPatient.date ? currentPatient.date : ""}`}
                                        style={{backgroundColor: "rgba(53, 162, 235, 0.5)"}}
                                    >
                                            ...
                                            <div className="col-md">
                                                <div className="d-flex">
                                                    <DatePicker
                                                        ...
                                                        format="MM-YYYY"
                                                        disabledDate={current => moment().add(0, "month")  < current}
                                                    />
                                                    <Button
                                                        className="ms-3"
                                            ...
                                            <div className="col-md">
                                                <div className="d-flex">
                                                    <DatePicker
                                                        ...
                                                        placeholder="Chọn tháng"
                                                        onChange={(e) => {
                                                            setMonthAppointment(moment(e?.$d).format("YYYY-MM"))
                                                        }}
                                                        format="MM-YYYY"
                                                    />
                                                    <Button
