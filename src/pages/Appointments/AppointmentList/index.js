...
export default function AppointmentList() {
			...
			render: (obj) => (
				<div>
					...
					<FontAwesomeIcon
						...
						hidden={
							newAppointment && obj.appointment_id === newAppointment
								? false
								: true
						}
					/>
	...
	//XỬ LÝ TÌM THEO THÔNG TIN BỆNH NHÂN
	const handleSearchByPatient = () => {
		if (keywordPatient) {
			const isPhoneNumber = CommonUtils.checkPhoneNumber(keywordPatient);
			if (isPhoneNumber) {
				const list = appointmentList.filter(
					(appointment) => appointment.phone === keywordPatient,
				);
				setSearchList(list);
				setKeywordPatient('');
			} else {
				const list = appointmentList.filter((appointment) => {
					return appointment.fullname
						.toLowerCase()
						.includes(keywordPatient.toLowerCase());
				});
				setSearchList(list);
				setKeywordPatient('');
			}
	...
	const handleSearchByDoctor = () => {
		if (keywordDoctor) {
			const isPhoneNumber = CommonUtils.checkPhoneNumber(keywordDoctor);
			if (isPhoneNumber) {
				const list = appointmentList.filter((appointment) => {
					return appointment.DoctorSchedule.Doctor.phone === keywordDoctor;
				});
				setSearchList(list);
				setKeywordDoctor('');
			} else {
				const list = appointmentList.filter((appointment) => {
					return appointment.DoctorSchedule.Doctor.fullname
						.toLowerCase()
						.includes(keywordDoctor.toLowerCase());
				});
				setSearchList(list);
				setKeywordDoctor('');
			}
	...
	const handleAcceptAppointment = () => {
		CustomSwal.fire({
			title: (
				<span>
					Xác nhận <b className="text-success">duyệt</b> lịch hẹn?
				</span>
			),
							...
							<Form layout="vertical">
								<div className="row mb-4">
									...
									<div className="col-md-4">
										<Form.Item label="Tên/SĐT người được khám">
											<div className="d-flex w-100">
												<Input
													...
													onKeyUp={handleEnter}
												/>
												<Button onClick={handleSearchByPatient}>Tìm</Button>
											</div>
										</Form.Item>
									</div>
									<div className="col-md-4">
										<Form.Item label="Tên/SĐT bác sĩ">
											<div className="d-flex w-100">
												<Input
													size="large"
													placeholder="Nhập thông tin"
													value={keywordDoctor}
													onChange={(e) => setKeywordDoctor(e.target.value)}
													onKeyUp={handleEnter}
												/>
					...
					<div className="row">
						<div className="col-md-4 mt-4 d-flex align-items-center justify-content-center">
							<p className="mb-0">
								...
								<span>
									<b
										className={`${
											!appointment.status ? 'text-danger' : 'text-muted'
										}`}
									>
						...
						{appointment.status !== 2 ? (
							<div className="col-md-4 mt-4 d-flex align-items-center justify-content-center">
								...
								<Button className="px-4" onClick={handleCancelAppointment}>
									Từ chối
								</Button>
							</div>
						) : (
